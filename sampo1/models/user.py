...
class User(Base):
    ...
    def group_role_ids(self):
        ...
        res = dbsession.execute(group_role_table.select(group_role_table.c.group_id.in_(grp_ids)).with_only_columns(
            [group_role_table.c.role_id]).distinct())
        ...
        if type(user) == int:
            if self.flags & self.f_composite_group:
                ...
                user_ids = [x[0] for x in
                            object_session(self).query(UserGroup.user_id).filter(UserGroup.group_id == self.id)
                            ]
